...
def _(mo):
    ...
    The data structures that represent a marimo project.
    Everything downstream — package building, documentation,
    bundling — is a traversal of these structures.

    Design principle: the data structure should make
    every algorithm that operates on it obvious. If you find
    yourself filtering, re-parsing, or reaching around the
    types, the types are wrong.
    ...
    Hierarchy:
        ...
        Project         — the whole thing (meta + modules)
          ...
          Module[]      — one per notebook file
            ...
            Setup[]     — other setup cell code
            Export[]    — decorated functions/classes
              Param[]   — parameters with inline docs
              Method[]  — class methods (if class)
              Return    — return annotation with inline doc
    ```

    **Note**: `Import`, `Const`, `Setup`, and `Raw` only need source text.
    Only `Export` needs the rich structure (params, methods, etc).
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Rename logic
     Lives here because parse needs it. No ast dependency.
     Pure string transform: prefix match → substitution.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Node types
    (each carries only what it needs)
    """)
    return
