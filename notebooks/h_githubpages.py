...
with app.setup:
    ...
    from fasthtml.components import ft,FT, Html, Head, Body, Header, Nav, Main, Aside, Footer, Div, H1, H2, H3, H4, H5, H6, P, A, Ul, Li, Img, Script, Link, Meta, Title, Strong, Code, Pre, Hr, Button, Section, Small, Input, Span, Iframe, Article, Style, NotStr, Details, Summary
    from fastcore.xml import to_xml
    from a_core import Node
    from b_read import scan, read_meta
    from d_docs import sig
...
@dataclass
class DocCtx:
    "Documentation context for rendering pages"
    pkg_name: str
    repo_url: str
    pypi_url: str
    base_url: str
...
def doc_ctx(root='.'):
    ...
    return DocCtx(
        ...
        base_url=f"https://{repo.split('github.com/')[1].split('/')[0]}.github.io/{repo.split('/')[-1]}/" if 'github.com' in repo else '',
...
@app.function
def render_head(title, ctx):
    return Head(
        Meta(charset='utf-8'),
        Meta(name='viewport', content='width=device-width, initial-scale=1'),
        Title(f'{title} - {ctx.pkg_name}'),
        Link(rel='stylesheet', href=f'{ctx.base_url}styles.css'),
...
def _(Icon):
    def render_header(ctx):
        return Header(id='header', cls='--make-lcr')(
            Div(cls='--make-cluster')(
                A(Icon('package'), Span(ctx.pkg_name), href=f'{ctx.base_url}index.html', cls='--make-cluster')),
            Input(type='search', placeholder='Search...', **{'data-bind': 'search'}),
            Div(cls='--make-cluster')(
                A(Icon('blocks'), href=ctx.pypi_url),
                A(Icon('github'), href=ctx.repo_url)))
    return (render_header,)


@app.function
def render_nav(ctx, current=''):
    return Nav(id='nav', cls='--make-stack')(
        A('Readme', href=f'{ctx.base_url}index.html', cls='active' if current == 'index' else ''),
        Hr(),
        *[A(name, href=f'{ctx.base_url}{name}.html', cls='active' if current == name else '') for name in ctx.module_names],
        Hr(),
        A('License', href=f'{ctx.base_url}license.html', cls='active' if current == 'license' else ''))


@app.cell
def _(render_header):
    def render_page(ctx, current, main_content):
        return Html(
            render_head(current, ctx),
            Body(cls='--make-stack')(
                render_header(ctx),
                Div(cls='--make-flank')(
                    render_nav(ctx, current),
                    Main(id='main', cls='--make-stack')(main_content))))
...
@app.function
def render_fn(node):
    sig = f"def {node.name}({', '.join(p.name for p in node.params)})"
    return Article(id=f'fn-{node.name}', cls='--make-stack')(
        Div(cls='--make-split')(
            H3(node.name),
            Code(node.kind.value)),
        P(node.doc) if node.doc else None,
        Pre(Code(sig, cls='language-python')))


@app.cell
def _(render_page):
    def render_module_page(ctx, mod_name, nodes):
        content = Div(cls='--make-stack')(
            H2(mod_name),
            *[render_fn(n) for n in nodes if n.kind.value in ('function', 'class')])
        return render_page(ctx, mod_name, content)
