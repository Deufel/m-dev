...
with app.setup:
    ...
    from html_tags import mktag, setup_svg, Tag, to_html
    Form = mktag('form')
...
def sanitize_for_js(text):
    """Sanitize text for safe embedding in a JS string"""
    return (text
...
@app.function
def render_node_with_search(n, repo_url=None, root='.'):
    t = 'class' if 'class ' in n.src else 'async' if n.src.lstrip().startswith('async ') else 'func'
    signature = clean(n.src)
    node_id = f"node-{n.module}-{n.name}".replace('.', '-')
    signal_search = f"searchable_{node_id}".replace('-', '_')
    signal_match  = f"match_{node_id}".replace('-', '_')
    tag_colors = {'func': '#10b981', 'async': '#f59e0b', 'class': '#8b5cf6'}

    # Sanitize searchable text to be safe in JS string
    raw_text = f"{n.name} {n.module} {t} {n.doc or ''} {signature}"
    searchable_text = sanitize_for_js(raw_text.lower())
    ...
    return Div(
        id=node_id,
        **{
            "data-signals":     f'{{"{signal_search}": "{searchable_text}", "{signal_match}": 0}}',
            "data-effect":      f'${signal_match} = [...$tags, $search.trim()].filter(t => t.length > 0 && ${signal_search}.toLowerCase().includes(t.toLowerCase())).length',
            "data-show":        f'($tags.length === 0 && $search.trim().length === 0) || ${signal_match} > 0',
            "data-style:order": f'($tags.length === 0 && $search.trim().length === 0) ? 0 : -${signal_match}',
        }
    )(
        Style()("""
            me { margin-bottom:0.75rem; border-radius:8px; overflow:hidden; background:#1e1e1e; position:relative; }
            me .match-badge { position:absolute; top:1rem; right:1rem; background:#3b82f6; color:white; padding:0.25rem 0.75rem; border-radius:9999px; font-size:0.8rem; font-weight:600; }
            me .header { display:flex; align-items:center; justify-content:space-between; padding:0.75rem 1rem; }
            me .header-left { display:flex; align-items:center; gap:0.75rem; }
            ...
            me .full-name { font-size:1rem; font-weight:500; }
            me .mod { color:#666; }
            me .name { color:#e5e5e5; }
            me .doc { margin:0; padding:0 1rem 0.5rem; color:#888; font-size:0.85rem; }
            ...
            me code { font-size:0.8rem; line-height:1.6; color:#ccc; counter-reset:line; }
        ...
        """),
        Span(cls="match-badge", **{
            "data-show": f'$tags.length > 0 || $search.trim().length > 0',
            "data-text": f'${signal_match}'
        })(),
        Div(cls="header")(
            Div(cls="header-left")(
                Span(cls="tag", style=f"background:{tag_colors.get(t,'#666')};")(t),
                Span(cls="full-name")(
                    Span(cls="mod")(f"{n.module}."),
                    Span(cls="name")(n.name)
                ) if n.module else Span(cls="full-name name")(n.name)
            ),
            Div(cls="header-right")(source_btn, edit_btn)
        ),
        P(cls="doc")(n.doc) if n.doc else None,
        Pre()(Code(cls="language-python", id=f"code-{node_id}")(*code_lines))
    )


@app.function
def render_module_page_datastar(mod_name, mod_nodes, all_mod_names, meta, root='.'):
    """Builds a Module Page with Datastar search"""
    ...
    head_elements = [
        ...
        Style()("""
            * { scrollbar-gutter:stable; box-sizing:border-box; }
            body { font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; margin:0; background:#121212; }
            code { font-family:'SF Mono',Consolas,monospace; }
            .layout { display:flex; height:100vh; }
            .sidenav { width:220px; background:#1a1a1a; padding:1rem; overflow-y:auto; display:flex; flex-direction:column; gap:0.25rem; flex-shrink:0; }
            .sidenav h3 { margin:0 0 0.75rem 0; color:#fff; font-size:0.9rem; text-transform:uppercase; }
            .nav-item { display:flex; align-items:center; justify-content:space-between; padding:0.4rem 0.5rem; border-radius:4px; color:#aaa; text-decoration:none; font-size:0.85rem; }
            .nav-item:hover { background:#252525; color:#fff; }
            .nav-item.active { color:#fff; background:#252525; }
            .nav-badge { background:#3b82f6; color:white; padding:0.1rem 0.4rem; border-radius:9999px; font-size:0.7rem; font-weight:600; }
            .nav-item.disabled { opacity:0.3; pointer-events:none; }
            .main { flex:1; display:flex; flex-direction:column; overflow:hidden; }
            .topbar { display:flex; align-items:center; justify-content:space-between; padding:0.75rem 1rem; background:#1e1e1e; border-bottom:1px solid #2a2a2a; gap:1rem; flex-shrink:0; }
            .topbar h1 { margin:0; font-size:1.25rem; color:#fff; }
            .search-bar { display:flex; gap:0.5rem; align-items:center; }
            .search-bar input { padding:0.5rem 0.75rem; border:1px solid #333; border-radius:4px; background:#252525; color:#fff; font-size:0.9rem; min-width:220px; }
            .search-bar input:focus { outline:2px solid #3b82f6; }
            .btn { padding:0.5rem 0.9rem; background:#3b82f6; color:white; border:none; border-radius:4px; cursor:pointer; font-size:0.85rem; }
            ...
            .tags-bar { display:flex; gap:0.5rem; flex-wrap:wrap; margin-bottom:0.5rem; min-height:1.5rem; }
            .tag-pill { background:#3b82f6; color:white; padding:0.2rem 0.6rem; border-radius:9999px; font-size:0.8rem; }
        """)
    ]

    nav_links = [
        Li()(A(href="index.html", cls="nav-item")("← Index")),
        *[
            Li()(A(
                href=f"{m}.html",
                cls=f"nav-item {'active' if m == mod_name else ''}",
            )(
                Span()(m),
                Span(cls="nav-badge", **{
            ...
            for m in all_mod_names
        ]
    ]

    body = Body(
        **{"data-signals": '{"search": "", "tags": []}', "style": "margin:0;"}
    )(
        Div(cls="layout")(
            Nav(cls="sidenav")(
                H3()(meta['name']),
                ...
                Div(cls="topbar")(
                    ...
                    Form(
                        ...
                        **{"data-on:submit__prevent": "$search.trim() ? ($tags = [...$tags, $search.trim()], $search = '') : null"}
                        ...
                        Input(type="text", placeholder="Search... (Enter to add tag)", **{"data-bind": "search"})(),
                        Button(cls="btn", type="submit")("Add Tag"),
                        Button(cls="btn clear", type="button", **{"data-on:click": "$tags = [], $search = ''"})("Clear")
                    )
                ),
                Div(cls="content")(
                    Div(cls="tags-bar", **{"data-show": "$tags.length > 0"})(),
                    Div(style="display:flex; flex-direction:column;")(
                        *[render_node_with_search(n, repo_url, root) for n in exp_nodes]
                    )
                )
            )
        )
...
def build_docs(root='.'):
    ...
    (docs_path / "index.html").write_text(to_html(render_index_page(meta, mods)))

    for mod_name, mod_nodes in mods:
        (docs_path / f"{mod_name}.html").write_text(
            to_html(render_module_page_datastar(mod_name, mod_nodes, mod_names, meta, root))
        )

    export_wasm(root)
    return f"Generated index + {len(mods)} module pages with Datastar search in {docs_path}"
