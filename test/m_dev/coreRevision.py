...
"""
"""
    "Extract param → {anno,doc} from inline # comments – only for functions"
    tree = ast.parse(src)
    node = tree.body[0]
    if not isinstance(node, ast.FunctionDef):
        return {}  # classes → no inline param comments supported
...
"""
"""
    "Raise `ValueError` if required keys missing from `meta`"
...
def scan(nb_dir:str='notebooks', style:str='nbdev')->ScanResult:
"""
"""
    "Scan marimo .py notebooks → metadata + exports"
    p = Path(nb_dir)
    meta, idx, mods = None, None, []
    for f in sorted(p.glob('*.py')):
        if f.name.startswith('.'): continue
        m, imps, exps, names = u_extract(f, style)
        if m: meta, idx = m, str(f)
    ...
    return {'metadata':meta, 'modules':mods, 'index_path':idx}
    ...
    for n in tree.body:
        if isinstance(n, ast.With):
            ...
            if any(u_is_export(d) for d in n.decorator_list):
                ...
                clean = u_clean(raw)
                final = clean if style=='nbdev' else u_google(clean)
...
"""
"""
    "Create `README.md` from `mo.md()` calls in index notebook"
    if not idx: return ''
...
def build(nb_dir:str='notebooks', out:str='src', style:str='nbdev')->str:
"""
"""
    "Turn marimo notebooks → clean installable package"
    ...
    for m in res['modules']:
        if m['name']=='index' or not m['exports']: continue
